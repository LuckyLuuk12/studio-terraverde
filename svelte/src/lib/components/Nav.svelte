<script lang="ts">
	...
	function toggleHamburger() {
		...
		for(let span: HTMLElement of hamburger.querySelectorAll('.line')) {
			if(count == 1) span.style.transform = unfolded ? 'rotate(45deg) translate(0.5rem, 0.5rem)' : 'rotate(0)';
...
<nav bind:this={nav}>
	<div class="hamburger" role="button" tabindex="0" bind:this={hamburger} on:click={toggleHamburger} on:keyup on:keydown={toggleHamburger}>
		<div class="line"></div>
		<div class="line"></div>
		<div class="line"></div>
	</div>
	<img src={favicon} alt="Company icon">
	<div class="hamburger-content" bind:this={hamburgerContent} style={style}>
		<a href="/">home</a>
		<div class="foldable">
			<div class="button" role="button" tabindex="0" on:click={(e) => {toggleFold(e)}} on:keyup on:keydown={(e) => {toggleFold(e)}}>
				<a href="/work">work</a>
